...
class MLMethodAssessment(Step):
    ...
    def run(input_params: MLMethodAssessmentParams = None):
        ...
        scores = MLMethodAssessment._score(metrics_list=input_params.metrics,
    ...
    @staticmethod
    def _score(metrics_list: set, optimization_metric: Metric, label: Label, predicted_y, predicted_proba_y_per_class,
               true_y, ml_score_path: Path,
               split_index: int, method: MLMethod):
        results = {}
        scores = {}

        predicted_proba_class = predicted_proba_y_per_class[
            label.name] if predicted_proba_y_per_class is not None else None
        predicted_proba_y = np.vstack(
            [predicted_proba_class[cls] for cls in label.values]).T if predicted_proba_class is not None else None
    ...
    @staticmethod
    def _store_predictions(method: MLMethod, true_y, predicted_y, predicted_proba_y_per_class, label: Label,
                           predictions_path, summary_path=None,
        ...
        for cls in method.get_classes():
            tmp = predicted_proba_y_per_class[label.name][cls] if predicted_proba_y_per_class is not None and \
                                                                  predicted_proba_y_per_class[
                                                                      label.name] is not None else None
