...
class ReflectionHandler:
    ...
    @staticmethod
    def _import_plugin_class(path: Path, class_name: str):
    ...
    def _import_class(path: Path, class_name: str):
        ...
        # TODO: check that the index of "immuneML" in path is on the correct place. The user may have the plugins in
        #  another folder with the same name
        if str(EnvironmentSettings.root_path) + "/immuneML/" not in str(path):
            cls = ReflectionHandler._import_plugin_class(path, class_name)
            return cls
        else:
    ...
    def _get_filenames(class_name: str, subdirectory_name: str = "", partial=False):
        ...
        # append all python files located in plugin directory to filenames
        for a in glob.glob(EnvironmentSettings.plugin_path + f"/**/*{class_name}.py"):
