...
class TestReceptorCNN(TestCase):
    ...
    def test_fit(self):
        ...
        dataset = RandomDatasetGenerator.generate_receptor_dataset(receptor_count=500,
                                                                                                  ...
                                                                                                  LabelConfiguration([
                                                                                                                         Label(
                                                                                                                             ...
                                                                                                                             [
                                                                                                                                 True,
                                                                                                                                 False])])))
        cnn = ReceptorCNN(kernel_count=2, kernel_size=[3], positional_channels=3, sequence_type="amino_acid",
                          device="cpu",
                          number_of_threads=4, random_seed=1, learning_rate=0.01, iteration_count=10,
                          l1_weight_decay=0.1, evaluate_at=5,
        ...
        self.assertListEqual(list(predictions_proba["CMV"][True] > 0.5),
                             [pred == True for pred in list(predictions["CMV"])])
