...
class TestProbabilisticBinaryClassifier(TestCase):
    ...
    def test_fit(self):
        ...
        proba_predictions = classifier.predict_proba(EncodedData(np.array([[6, 7], [1, 6]])),
                                                     Label("cmv", [True, False]))
        ...
        self.assertTrue(
            (proba_predictions["cmv"][False] <= 1.0).all() and (proba_predictions["cmv"][False] >= 0.0).all())

        self.assertListEqual(list(proba_predictions["cmv"][True] > 0.5),
